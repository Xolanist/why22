...
impl From<why_store::Error> for Error {
	fn from(e: why_store::Error) -> Error {
		Error::Store(e)
	}
}
...
impl PeerAddr {
	...
	/// defaults to port 3514 on mainnet and 13614 on testnet.
	pub fn from_ip(addr: IpAddr) -> PeerAddr {
		let port = if global::is_testnet() { 13614 } else { 3514 };
...
/// included in why-server.toml, but we don't want them to ever return none
impl P2PConfig {
	/// return ban window
