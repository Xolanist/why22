...
//! Modified from above to integrate into why and allow for different
...
/// Why uses blake2 everywhere but the spec calls for SHA512/Ripemd160, so allow
...
impl BIP32WhyHasher {
	/// New empty hasher
	pub fn new(is_test: bool) -> BIP32WhyHasher {
		BIP32WhyHasher {
			is_test: is_test,
...
impl BIP32Hasher for BIP32WhyHasher {
	fn network_priv(&self) -> [u8; 4] {
		if self.is_test {
			[0x03, 0x27, 0x3A, 0x10]
		} else {
...
impl ExtendedPrivKey {
	...
	pub fn from_mnemonic(
		...
		let mut hasher = BIP32WhyHasher::new(is_test);
