...
pub const SERVER_CONFIG_FILE_NAME: &str = "why-server.toml";
const SERVER_LOG_FILE_NAME: &str = "why-server.log";
const GRIN_HOME: &str = ".why";
...
fn get_why_path(chain_type: &global::ChainTypes) -> Result<PathBuf, ConfigError> {
	// Check if why dir exists
	let mut why_path = match dirs::home_dir() {
		Some(p) => p,
		None => PathBuf::new(),
	};
	why_path.push(GRIN_HOME);
	why_path.push(chain_type.shortname());
	// Create if the default path doesn't exist
	if !why_path.exists() {
		fs::create_dir_all(why_path.clone())?;
	}
	Ok(why_path)
}
...
fn check_api_secret_files(
	...
	let why_path = get_why_path(chain_type)?;
	let mut api_secret_path = why_path;
...
pub fn initial_setup_server(chain_type: &global::ChainTypes) -> Result<GlobalConfig, ConfigError> {
	...
	// Use config file if current directory if it exists, .why home otherwise
	if let Some(p) = check_config_current_dir(SERVER_CONFIG_FILE_NAME) {
		...
		let mut config_path = why_path.clone();
		...
		if !config_path.exists() {
			...
			default_config.update_paths(&why_path);
...
impl GlobalConfig {
	...
	pub fn for_chain(chain_type: &global::ChainTypes) -> GlobalConfig {
		...
		match *chain_type {
			...
			global::ChainTypes::Testnet => {
				defaults.api_http_addr = "127.0.0.1:13613".to_owned();
				defaults.p2p_config.port = 13614;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:13616".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:13615".to_owned();
			}
			global::ChainTypes::UserTesting => {
				defaults.api_http_addr = "127.0.0.1:23613".to_owned();
				defaults.p2p_config.port = 23614;
				defaults.p2p_config.seeding_type = p2p::Seeding::None;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:23616".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:23615".to_owned();
			}
	...
	pub fn update_paths(&mut self, why_home: &PathBuf) {
		...
		let mut foreign_api_secret_path = why_home.clone();
		...
		let mut log_path = why_home.clone();
