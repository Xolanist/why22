# Why - Instruction of Release

**Note**: *[totally draft doc! to be reviewed and discussed]*

## Version Number Rule

In Why, we're using [Semantic Versioning 2.0.0](https://semver.org). For a short description of the rule:
...
The examples of the release version of Why:
...
In Why, **build metadata** is used as the build number which comes from the Travis-CI job ID of building jobs, it's an unique ID for each building. **Note**: for the moment, this metadata is only used in the name of the released binary, and it's auto generated, no need to set it manually.

Here is an example of the build metadata of Why release version:
...
So far, Why support both Mac and Linux, and 64bits only. There're 2 binaries in one version release, plus md5 checksum for each binary.

For example:

- why-0.3.1-pre.1-430839316-oxs.tgz
- why-0.3.1-pre.1-430839316-oxs.tgz-md5sum.txt
- why-0.3.1-pre.1-430839318-linux-amd64.tgz
- why-0.3.1-pre.1-430839318-linux-amd64.tgz-md5sum.txt
...
Each time when Why release a new version, there's definitely a tag there with same version number. But it's not mandatory to have a **branch** with this version release.
