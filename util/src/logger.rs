...
impl Default for LoggingConfig {
	fn default() -> LoggingConfig {
		LoggingConfig {
			...
			log_file_path: String::from("why.log"),
...
/// This filter is rejecting messages that doesn't start with "why"
/// in order to save log space for only Why-related records
#[derive(Debug)]
struct WhyFilter;

impl Filter for WhyFilter {
	fn filter(&self, record: &Record<'_>) -> Response {
		if let Some(module_path) = record.module_path() {
			if module_path.starts_with("why") {
