...
pub fn amount_from_hr_string(amount: &str) -> Result<u64, Error> {
	...
	let (whys, nwhys) = match amount.find('.') {
		None => (parse_whys(amount)?, 0),
		Some(pos) => {
			let (gs, tail) = amount.split_at(pos);
			(parse_whys(gs)?, parse_nwhys(&tail[1..])?)
		}
	};
	Ok(whys * GRIN_BASE + nwhys)
}

fn parse_whys(amount: &str) -> Result<u64, Error> {
	if amount == "" {
		Ok(0)
	} else {
		amount
...
fn parse_nwhys(amount: &str) -> Result<u64, Error> {
