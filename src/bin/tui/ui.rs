...
impl UI {
	...
	pub fn new(
		...
		//set theme
		let mut theme = why_ui.cursive.current_theme().clone();
		modify_theme(&mut theme);
		why_ui.cursive.set_theme(theme);
		why_ui.cursive.add_fullscreen_layer(main_layer);

		// Configure a callback (shutdown, for the first test)
		let controller_tx_clone = why_ui.controller_tx.clone();
		why_ui.cursive.add_global_callback('q', move |c| {
