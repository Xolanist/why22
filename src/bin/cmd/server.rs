...
/// Why server commands processing
...
use why_p2p::msg::PeerAddrs;
use why_p2p::PeerAddr;
...
/// stopping the Why blockchain server. Processes all the command line
/// arguments to build a proper configuration and runs Why with that
/// configuration.
pub fn server_command(
	...
	if let Some(a) = server_args {
		match a.subcommand() {
			...
			("", _) => {
				println!("Subcommand required, use 'why help server' for details");
			}
			(cmd, _) => {
				println!(":: {:?}", server_args);
				panic!(
					"Unknown server command '{}', use 'why help server' for details",
					cmd
				);
			}
		}
